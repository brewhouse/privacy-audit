...
- **Cookieless analytics is reported, not penalised.** Plausible, Fathom, Umami and similar set no cookie, carry no cross-site identifier and retain no personal data. Mark them `cookieless: true` in the vendor map: they stay in the inventory and get their own LOW finding, but are excluded from violation counts and the score. Otherwise removing a genuine tracker could *lower* a client's score, and a client who deliberately chose the privacy-preserving tool would be penalised for it.
